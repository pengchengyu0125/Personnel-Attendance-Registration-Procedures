...
public interface PageMapper {
    ...
    //根据查询条件查询内容，名字为空
    @Select("select h.*,i.status from t_mg_visit_infodept_history AS h LEFT JOIN t_mg_visit_infodept_info AS i ON h.visit_phone = i.visit_phone where h.visit_phone regexp #{visitPhone} and h.visit_time >= #{startTime} and h.visit_time <= #{endTime} order by h.create_time desc limit #{offset},#{size}")
    ...
    //根据查询条件查询内容，手机号为空
    @Select("select h.*,i.status from t_mg_visit_infodept_history AS h LEFT JOIN t_mg_visit_infodept_info AS i ON h.visit_phone = i.visit_phone where h.visit_name regexp #{visitName} and h.visit_time >= #{startTime} and h.visit_time <= #{endTime} order by h.create_time desc limit #{offset},#{size}")
    ...
    @Select("select h.*,i.status from t_mg_visit_infodept_history AS h LEFT JOIN t_mg_visit_infodept_info AS i ON h.visit_phone = i.visit_phone where h.visit_time >= #{startTime} and h.visit_time <= #{endTime} order by h.create_time desc limit #{offset},#{size}")
